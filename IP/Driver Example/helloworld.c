...
void configure_Write_AXI(volatile uint32_t* slaveport, uint64_t addressSpaceOffset, uint32_t addressSpaceSize, int burstSize, int offsetBetweenBurstLocations)
{
	...
	*(p++) = offsetBetweenBurstLocations + 16*burstSize;
	*(p++) = addressSpaceSize/(offsetBetweenBurstLocations + 16*burstSize);
	*(p++) = burstSize;
	*(p++) = 1;
	*(p++) = 5;
}
...
void print_AXI(volatile uint32_t* slaveport, double frequency){
	 ...
	 * latency holds a number that: latency/readOps = average latency. latency/runtime = average amount of read requests in flight
	...
	uint64_t rb = *(p+16);
	printf("Test Finished in %.3f sec.\n\rWrite Speed: %.3f MB/s \n\rRead Speed %.3f MB/s \n\rAvg. Read Latency: %.3f clock cycles \n\rMax Read AXI inactivity: %lu clock cycles\n\r",runtime/frequency,(writeOps*(wb*16))/(runtime/frequency)/(1000000.0),(readOps*(wb*16))/(runtime/frequency)/(1000000.0),(latency * 1.0)/readOps,inactive);
}
int main()
{
    init_platform();
    ...
    /* For ease we simply allocate 3/4 of the memory and then use it in the middle*/
    int* malloc1_5G = malloc(1536*1024*1024);
    reset_AXI(HP3);
    //reset_AXI(HP2);
    //reset_AXI(HP1);
    reset_AXI(HP0);

    /* Placement matters and this is the placement we have used in Ultra96. We did not have time to evaluate placement (wrt bank/rank/bank group). Hopefully somebody else can research that. */

    /* Start both reads and writes on HP3 to have a 128MB address space each, operate sequentially on 8*16=128 Byte bursts */
    ...
    /* Start both reads and writes on HP0 to have a 128MB address space each, operate sequentially on 8*16=128 Byte bursts */
